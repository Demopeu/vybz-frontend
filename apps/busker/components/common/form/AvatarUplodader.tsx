...
export default function AvatarUploader({ userUuid }: { userUuid: string }) {
  const { buskerProfileUrl } = use(FormContext);
  const { setBuskerProfileUrl } = use(FormContext);
  const [previewSrc, setPreviewSrc] = useState<string>(buskerProfileUrl);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const { uploadFile, error } = useS3Uploader({
    onSuccess: (url) => {
      setPreviewSrc(url);
      setBuskerProfileUrl?.(url);
    },
    onError: (err) => {
