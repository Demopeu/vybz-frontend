...
function generateS3Key({
  ...
  return `uploads/${type}/${date}/${userId}-${uuid}.${ext}`;
}
...
export async function POST(req: NextRequest) {
  try {
    const { type, userId, originalName, fileType, fileContent } = await req.json();
    const s3Key = generateS3Key({ type, userId, originalName });
